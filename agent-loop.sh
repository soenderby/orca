...
ORCA_HOME="${ORCA_HOME:-$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)}"
WORKTREE="${WORKTREE:-}"
...
# Prompt: target repo first, fall back to orca home.
if [[ -z "${PROMPT_TEMPLATE:-}" ]]; then
  if [[ -f "${ROOT}/ORCA_PROMPT.md" ]]; then
    PROMPT_TEMPLATE="${ROOT}/ORCA_PROMPT.md"
  else
    PROMPT_TEMPLATE="${ORCA_HOME}/ORCA_PROMPT.md"
  fi
fi

PRIMARY_REPO="${ORCA_PRIMARY_REPO:-${ROOT}}"
LOCK_HELPER_PATH="${ORCA_WITH_LOCK_PATH:-${ORCA_HOME}/with-lock.sh}"
QUEUE_READ_HELPER_PATH="${ORCA_QUEUE_READ_MAIN_PATH:-${ORCA_HOME}/queue-read-main.sh}"
QUEUE_WRITE_HELPER_PATH="${ORCA_QUEUE_WRITE_MAIN_PATH:-${ORCA_HOME}/queue-write-main.sh}"
MERGE_HELPER_PATH="${ORCA_MERGE_MAIN_PATH:-${ORCA_HOME}/merge-main.sh}"
MAX_RUNS="${MAX_RUNS:-0}"
