...
if [[ ! -x "${ORCA_BIN_CANDIDATE}" ]]; then
  echo "[merge-main] error: orca binary not found: ${ORCA_BIN_CANDIDATE}" >&2
  echo "[merge-main] build it first: go build -o ${SCRIPT_DIR}/orca ./cmd/orca" >&2
  exit 1
fi

exec "${ORCA_BIN_CANDIDATE}" merge-main "$@"
