...
Commands:
  --follow Emit merged managed+observed lifecycle JSONL stream
...
start_managed_follow_stream() {
  ...
  if [[ -n "${SESSION_FILTER_PREFIX}" ]]; then
    cmd+=(--session-prefix "${SESSION_FILTER_PREFIX}")
  fi
  ...
  while [[ $# -gt 0 ]]; do
    case "$1" in
      ...
      -h|--help)
        ...
        ;;
      *)
        invalid "unknown option for monitor --follow: $1"
        ;;
    esac
    shift
  done
