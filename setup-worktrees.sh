...
if [[ ! -x "${ORCA_BIN_CANDIDATE}" ]]; then
  echo "[setup-worktrees] error: orca binary not found: ${ORCA_BIN_CANDIDATE}" >&2
  echo "[setup-worktrees] build it first: go build -o ${SCRIPT_DIR}/orca ./cmd/orca" >&2
  exit 1
fi

exec "${ORCA_BIN_CANDIDATE}" setup-worktrees "$@"
